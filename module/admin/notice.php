...
switch ($act) {
	...
	case 'edit':
		...
		if (isset($_p_pesubmit)) {
			pe_token_match();
			if ($db->pe_update('notice', array('notice_id'=>$notice_id), pe_dbhold($_p_info, array('notice_emailtext')))) {
				cache_write('notice');
	...
	case 'state':
		pe_token_match();
		if ($db->pe_update('notice', array('notice_id'=>is_array($_p_notice_id) ? $_p_notice_id : $_g_id), array('notice_state'=>$_g_state))) {
			cache_write('notice');
			pe_success("操作成功!");
		}
		else {
	...
	//#####################@ 发送记录 @#####################//
	case 'log':
		$info_list = $db->pe_selectall('noticelog', array('order by'=>'noticelog_id desc'), '*', array(20, $_g_page));
		$seo = pe_seo($menutitle='邮件发送记录', '', '', 'admin');
